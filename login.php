	...
	if ($count == 1) {
		...
		if (isset($_COOKIE["product_list"])) {
			...
			for ($i = 0; $i < count($product_list); $i++) {
				...
				$verify_cart = "SELECT id FROM ligne_commande WHERE client_id = $_SESSION[uid] AND p_id = " . $product_list[$i];
				$result  = mysqli_query($con, $verify_cart);
				if (mysqli_num_rows($result) < 1) {
					//if user is adding first time product into cart we will update user_id into database table with valid id
					$update_cart = "UPDATE ligne_commande SET client_id = '$_SESSION[uid]' WHERE ip_add = '$ip_add' AND client_id = -1";
					mysqli_query($con, $update_cart);
				} else {
					//if already that product is available into database table we will delete that record
					$delete_existing_product = "DELETE FROM ligne_commande WHERE client_id = -1 AND ip_add = '$ip_add' AND p_id = " . $product_list[$i];
