$(document).ready(function () {
		...
		$.ajax({
			...
			success: function (response) {
				...
				$.each(resp.message, function (index, value) {
					brandHTML += '<tr>' +
						'<td></td>' +
						'<td>' + value.nom_marque + '</td>' +
						'<td><a class="btn btn-sm btn-info edit-brand"><span style="display:none;">' + JSON.stringify(value) + '</span><i class="fas fa-pencil-alt"></i></a>&nbsp;<a bid="' + value.marque_id + '" class="btn btn-sm btn-danger delete-brand"><i class="fas fa-trash-alt"></i></a></td>' +
						'</tr>';
				});
	...
	$(document.body).on('click', '.delete-brand', function () {
		...
		if (confirm("vous voulez vraiment supprimer cette marque ?")) {
			$.ajax({
				url: '../admin/classes/Products.php',
				method: 'POST',
